...
{
	EditorTextureViewerWindow::EditorTextureViewerWindow(const std::string& name)
		: GUIWindow(name)
		, mTex(nullptr)
		, mSize(800, 600)
	{
	...
	EditorTextureViewerWindow::~EditorTextureViewerWindow()
	{
		mTex->deleteObject();
	}

	void EditorTextureViewerWindow::setTexture(Texture* tex)
	{
		mTex = tex;
		mTex->saveObject();
	}

	void EditorTextureViewerWindow::loadTexture(const std::filesystem::path& path)
	{
		if (mTex)
		{
			mTex->deleteObject();
	...
	bool EditorTextureViewerWindow::begin()
	{
		return ImGui::Begin(mName.c_str(), &mIsOpen, ImGuiWindowFlags_NoScrollbar);
	}

	void EditorTextureViewerWindow::onRender()
	{
		GUIWindow::onRender();
