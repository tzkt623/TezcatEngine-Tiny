...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::GLSL_TINY(const std::vector<const char*>& myKeyWords, const std::vector<const char*>& myIdentifiers)
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		const char* const keywords[] =
		{
			...
			, "bool", "str", "float", "int", "double", "short", "long"
		...
		const char* const identifiers[] =
		{
			...
			, "floor", "fmod", "getchar", "getenv", "isalnum", "isalpha"
			, "isdigit", "isgraph", "ispunct", "isspace", "isupper", "kbhit"
			, "log10", "log2", "log", "memcmp", "modf", "pow", "putchar", "putenv"
			, "puts", "rand", "remove", "rename", "sinh", "sqrt", "srand", "strcat"
			, "strcmp", "strerror", "time", "tolower", "toupper"
			, "texture","textureLod", "normalize", "cross", "dot", "pow", "max", "min"
		};

		for (auto& k : identifiers)
		{
			Identifier id;
			id.mDeclaration = "GLSL_TINY Built-in";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
		...
		{
			Identifier id;
			id.mDeclaration = "GLSL_TINY Build-in";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
