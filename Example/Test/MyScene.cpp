...
void MyScene::createTransparentObject()
{
	std::random_device rd;
	std::mt19937 gen(rd());
...
void MyScene::createPBR()
{
	...
	auto mr = go->addComponent<MeshRenderer>();
	auto material = new Material("Standard/PBRStd1");
	...
	auto go2 = new GameObject("PBRBall2");
	go2->addComponent<Transform>();
	go2->getTransform()->setPosition(glm::vec3(-20.0f, 0.0f, -20.0f));
	go2->getTransform()->setScale(glm::vec3(10.0f));
	go2->getTransform()->setRotation(glm::vec3(90.0f, 0.0f, 0.0f));
	...
	for (int y = 0; y <= 5; y++)
	{
		for (int x = 0; x <= 5; x++)
		{
			...
			auto mr = go->addComponent<MeshRenderer>();
			auto material = new Material("Standard/PBRTest1");
